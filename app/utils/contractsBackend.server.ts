...
// Utility: ensure value is a parsed JSON object or fallback to default
const safeJsonParse = (value: any, fallback: Record<string, any> = {}) => {
  ...
  export async function getCustomerSchemaFromSaleId(
    ...
    // 1. Fetch core sale & customer information
    ...
    slabs.forEach((slab) => {
      ...
      if (!roomsMap[roomId]) {
        roomsMap[roomId] = {
          ...
          slabs: [],
          extras: safeJsonParse(slab.extras),
        };
      }
