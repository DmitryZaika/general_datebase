...
export async function listThreadsForUser(
  ...
  `;
	...
	const unreadCount = await getUnreadThreadCountForUser(
		user,
		scope,
		filterAgentId,
	);
	...
	const threads: ThreadSummaryRow[] = dedupedRows.map((r) => {
		...
		return {
			...
			lastDirection: inferSmsDirection(
				r.last_sender,
...
function mapRowToExpanded(r: ThreadMessageRow): SmsRowExpanded {
	return {
		...
		sender:
			r.sender === null || r.sender === undefined ? null : String(r.sender),
