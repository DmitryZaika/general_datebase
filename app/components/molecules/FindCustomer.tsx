        ...
        {isInputFocused && customers.length > 0 && (
          ...
          >
            {displayCustomers.map(customer => {
              ...
              const phoneLine = phoneTrim || 'no phone number'
              return (
                <div
                  key={customer.id}
                  className='flex justify-between gap-2 border-b border-gray-100 p-1.5 last:border-none hover:bg-blue-50 items-start'
                >
                  <div className='flex min-w-0 flex-1 flex-col gap-0.5 items-start'>
                    <button
                      type='button'
                      className='font-medium text-gray-800 hover:underline cursor-pointer text-left focus:outline-none focus:ring-2 focus:ring-blue-500 rounded'
                      onClick={e => {
                        e.stopPropagation()
                        if (onSelect) {
                    ...
                    <div className='flex flex-col text-sm text-left w-full'>
                      <p className={emailTrim ? 'text-gray-500' : 'text-gray-400'}>
                        {emailLine}
                      </p>
                      <p className={phoneTrim ? 'text-gray-500' : 'text-gray-400'}>
                        {phoneLine}
                      </p>
                      {customer.phone_2?.trim() ? (
                        <p className='text-gray-500'>{customer.phone_2.trim()}</p>
                      ) : null}
                    </div>
                  </div>
                  {showActions && (
