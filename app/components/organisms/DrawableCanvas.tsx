...
export function DrawableCanvas({ width = 800, height = 1000, onSubmit }: DrawableCanvasProps) {
    ...
    const handleMouseMove: React.MouseEventHandler<SVGSVGElement> = (e) => {
        ...
        const raw = { x: (e.clientX - rect.left) / scale, y: (e.clientY - rect.top) / scale }
        setHoverRaw(raw)
    }

    const handleClick: React.MouseEventHandler<SVGSVGElement> = (e) => {
        const rect = e.currentTarget.getBoundingClientRect()
        const curr = { x: (e.clientX - rect.left) / scale, y: (e.clientY - rect.top) / scale }
        if (!anchor) {
        ...
        if (anchor.shapeIndex !== undefined) {
            ...
            const s = shapes[si]
            if (s && s.kind === 'rect') {
                const colinear = (s.a.y === s.b.y && isHorizontal) || (s.a.x === s.b.x && !isHorizontal)
                if (colinear && perpMag === 0) {
                    const next = { ...s }
                    if (anchor.which === 'a') next.a = b; else next.b = b
                    const arr = shapes.slice(); arr[si] = next; setShapes(arr)
                    setAnchor(null); setHoverRaw(null); return
                } else {
                    const start = anchor.which === 'a' ? s.a : s.b
                    const mid = axisIsHorizontal ? { x: curr.x, y: start.y } : { x: start.x, y: curr.y }
                    const end = isHorizontal ? { x: mid.x, y: curr.y } : { x: curr.x, y: mid.y }
                    setShapes(arr => [...arr, { kind: 'l', a: start, b: mid, c: end, width: s.width }])
                    setAnchor(null); setHoverRaw(null); return
                }
            }
        }
        ...
        setAnchor(null); setHoverRaw(null)
    }
    ...
    const shapePolygons = useMemo(() => {
        return shapes.map(s => s.kind === 'rect' ? buildRectPolygonCentered(s.a, s.b, s.width) : buildLPolygonCentered(s.a, s.b, s.c, s.width))
    }, [shapes])
            ...
            <div className="w-full h-full">
                ...
                <g transform={`scale(${scale})`}>
                    {shapePolygons.map((poly, si) => (
                        <polygon key={`poly-${si}`} points={poly.map(p => `${p.x},${p.y}`).join(" ")} fill="rgba(59,130,246,0.15)" stroke="#3b82f6" strokeWidth={strokeW} />
                    ))}
                    {outlinePoints ? (
                        <polygon points={outlinePoints} fill="rgba(59,130,246,0.10)" stroke="#3b82f6" strokeWidth={strokeW} />
                    ) : null}
                    {shapePolygons.map((poly, si) => {
                        const cx = poly.reduce((s, p) => s + p.x, 0) / poly.length
                        const cy = poly.reduce((s, p) => s + p.y, 0) / poly.length
                        const segs: { a: Point; b: Point; idx: number }[] = []
                        for (let i = 1; i < poly.length; i++) segs.push({ a: poly[i - 1], b: poly[i], idx: i - 1 })
                        ...
                        return segs.map((seg, idx) => {
                            ...
                            return (
                                <text
                                    ...
                                    onClick={(ev) => {
                                        ev.stopPropagation()
                                        setEditing({ shapeIndex: si, segmentIndex: seg.idx, left: tx * scale, top: ty * scale, value: length.toFixed(1) })
                                    }}
                        ...
                        onBlur={() => {
                            ...
                            if (shape.kind === 'rect') {
                                ...
                                if (dotPrimary >= dotNormal) {
                                    ...
                                    const next = { kind: 'rect', a, b: newB, width: shape.width } as RectShape
                                    const arr = shapes.slice()
                                    arr[si] = next
                                    setShapes(arr)
                                } else {
                                    const next = { kind: 'rect', a: shape.a, b: shape.b, width: num } as RectShape
                                    const arr = shapes.slice()
                                    arr[si] = next
                                    setShapes(arr)
                                }
                                ...
                                if (dPrimary >= dSecondary && dPrimary >= dWidth) {
                                    ...
                                    const next = { kind: 'l', a, b: newB, c: shape.c, width: shape.width } as LShape
                                    const arr = shapes.slice()
                                    arr[si] = next
                                    setShapes(arr)
                                    ...
                                    const next = { kind: 'l', a, b, c: newC, width: shape.width } as LShape
                                    const arr = shapes.slice()
                                    arr[si] = next
                                    setShapes(arr)
                                } else {
                                    const next = { kind: 'l', a: shape.a, b: shape.b, c: shape.c, width: num } as LShape
                                    const arr = shapes.slice()
                                    arr[si] = next
                                    setShapes(arr)
                                }
