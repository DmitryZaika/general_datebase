...
function buildLPolygonCentered(a: Point, b: Point, c: Point, width: number): Point[] {
  ...
  const half = width / 2
  const outerCorner = {
    x: b.x - nx1 * half - nx2 * half,
    y: b.y - ny1 * half - ny2 * half,
  }
  const innerCorner = {
    x: b.x + nx1 * half + nx2 * half,
    y: b.y + ny1 * half + ny2 * half,
...
function computeRectPreview(
  ...
  const perpendicular =
    axis === 'h' ? Math.abs(hover.y - reference.y) : Math.abs(hover.x - reference.x)
  const corner =
    axis === 'h' ? { x: aligned.x, y: hover.y } : { x: hover.x, y: aligned.y }
...
function computePreviewShape(
      ...
      const locked = turnLock
        ...
        : dragAxis
          ? anchor.which === 'b'
            ...
            : base.a
          : null
      const preview = computeRectPreview(
        base,
        anchor.which === 'b' ? 'b' : 'a',
        hover,
        dragAxis,
  ...
  const axis = turnLock
    ? turnLock.axis
    : (dragAxis ?? getAxisFromPoints(anchor.point, hover))
  const alignedBase = alignPoint(hover, anchor.point, axis)
  const aligned = turnLock ? turnLock.point : alignedBase
  const corner =
    axis === 'h' ? { x: aligned.x, y: hover.y } : { x: hover.x, y: aligned.y }
  const perpendicular =
    axis === 'h' ? Math.abs(corner.y - aligned.y) : Math.abs(corner.x - aligned.x)
  if (perpendicular >= TURN_THRESHOLD) {
    return {
      shape: { kind: 'l', a: anchor.point, b: aligned, c: corner, width: defaultWidth },
      axis,
      aligned,
    }
  }
  return {
    shape: { kind: 'rect', a: anchor.point, b: aligned, width: defaultWidth },
    axis,
    aligned,
  }
}

const pointsToPolygon = (points: Point[]): string =>
  points.map(p => `${p.x},${p.y}`).join(' ')

const nearlyEqual = (a: number, b: number, tolerance = 0.001) =>
...
const buildPreviewPolygon = (shape: Shape): Point[] =>
  shape.kind === 'rect'
    ? buildRectPolygonCentered(shape.a, shape.b, shape.width)
    : buildLPolygonCentered(shape.a, shape.b, shape.c, shape.width)
...
export function DrawableCanvas({
  ...
  const panStartRef = useRef<{ sx: number; sy: number; ox: number; oy: number } | null>(
    null,
  )
  ...
  const handleMouseMove: React.MouseEventHandler<SVGSVGElement> = e => {
    ...
    if (draggingShape && dragStartRef.current) {
      ...
      const updated: Shape =
        original.kind === 'rect'
          ? {
              kind: 'rect',
              a: { x: original.a.x + dxWorld, y: original.a.y + dyWorld },
              b: { x: original.b.x + dxWorld, y: original.b.y + dyWorld },
          ...
          : {
              ...
              a: { x: original.a.x + dxWorld, y: original.a.y + dyWorld },
              b: { x: original.b.x + dxWorld, y: original.b.y + dyWorld },
              c: { x: original.c.x + dxWorld, y: original.c.y + dyWorld },
              width: original.width,
            }
    ...
    let hh: { shapeIndex: number; which: 'a' | 'b' | 'c'; axis: 'h' | 'v' } | null =
      null
    for (let i = 0; i < shapes.length; i++) {
      ...
      if (s.kind === 'rect') {
        ...
        const axis: 'h' | 'v' =
          Math.abs(s.b.y - s.a.y) < Math.abs(s.b.x - s.a.x) ? 'h' : 'v'
        if (nearA) {
          hh = { shapeIndex: i, which: 'a', axis }
          break
        }
        if (nearB) {
          hh = { shapeIndex: i, which: 'b', axis }
          break
        }
      } else {
        const nearA = Math.hypot(raw.x - s.a.x, raw.y - s.a.y) <= threshold
        ...
        const axisA: 'h' | 'v' =
          Math.abs(s.b.y - s.a.y) < Math.abs(s.b.x - s.a.x) ? 'h' : 'v'
        const axisC: 'h' | 'v' =
          Math.abs(s.c.y - s.b.y) < Math.abs(s.c.x - s.b.x) ? 'h' : 'v'
        if (nearA) {
          hh = { shapeIndex: i, which: 'a', axis: axisA }
          break
        }
        if (nearC) {
          hh = { shapeIndex: i, which: 'c', axis: axisC }
          break
        }
      }
    }
    ...
    if (anchor) {
      ...
      const referencePoint =
        anchor.shapeIndex !== undefined
          ? (() => {
              const anchoredShape = shapes[anchor.shapeIndex]
              if (!anchoredShape) return anchor.point
              if (anchoredShape.kind === 'rect')
                return anchor.which === 'b' ? anchoredShape.b : anchoredShape.a
      ...
      const perp =
        axis === 'h'
          ? Math.abs(raw.y - referencePoint.y)
          : Math.abs(raw.x - referencePoint.x)
      if (!turnLock && perp >= TURN_THRESHOLD) {
        const aligned = alignPoint(raw, referencePoint, axis)
        const side: TurnLock['side'] =
          anchor.shapeIndex !== undefined ? (anchor.which === 'b' ? 'b' : 'a') : 'start'
  ...
  const handleClick: React.MouseEventHandler<SVGSVGElement> = e => {
    ...
    const preview = computePreviewShape(
      shapes,
      anchor,
      curr,
      dragAxis,
  ...
  const previewInfo = useMemo(
    () => computePreviewShape(shapes, anchor, hoverRaw, dragAxis, RECT_WIDTH, turnLock),
    [shapes, anchor, hoverRaw, dragAxis, turnLock],
  )

  const previewPoints: Point[] = useMemo(
    () => (previewInfo ? buildPreviewPolygon(previewInfo.shape) : []),
    [previewInfo],
  )
  ...
  return (
    <div
      ...
      style={{ height: availableH ? `${availableH}px` : '100%' }}
    >
          ...
          <g transform={`translate(${pan.x},${pan.y}) scale(${scale})`}>
            {shapes.map((shape, index) => {
              ...
              const basePoly =
                shape.kind === 'rect'
                  ...
                  : buildLPolygonCentered(shape.a, shape.b, shape.c, shape.width)
              const poly =
                isActive && previewInfo
                  ? buildPreviewPolygon(previewInfo.shape)
                  : basePoly
              return (
                <polygon
                  key={`poly-${index}`}
            ...
            {previewInfo
              ? (() => {
                  ...
                  const cx =
                    previewPoly.reduce((s, p) => s + p.x, 0) / previewPoly.length
                  const cy =
                    previewPoly.reduce((s, p) => s + p.y, 0) / previewPoly.length
                  const segs: { a: Point; b: Point }[] = []
                  for (let i = 1; i < previewPoly.length; i++)
                    segs.push({ a: previewPoly[i - 1], b: previewPoly[i] })
                  segs.push({
                    a: previewPoly[previewPoly.length - 1],
                    b: previewPoly[0],
                  })
                  return segs.map((seg, idx) => {
                    const midX = (seg.a.x + seg.b.x) / 2
                    ...
                    const dlen = Math.hypot(dirToOutsideX, dirToOutsideY) || 1
                    const off =
                      (previewInfo.shape.kind === 'rect'
                        ? previewInfo.shape.width
                        : previewInfo.shape.width) + 12
