...
export default function Samples() {
  ...
  const columns: ColumnDef<Stone>[] = [
    ...
    },
    {
      ...
      cell: ({ row }) => {
        return <div className='font-medium cursor-pointer'>{row.original.name}</div>
      },
    },
    {
      accessorKey: 'amount',
        ...
        return (
          <button
            ...
            onClick={async e => {
              ...
              setSortedStones(prev =>
                prev.map(s =>
                  s.id === stone.id ? { ...s, samples_amount: newAmount } : s,
                ),
              )
            }}
          >
  ...
  return (
    <>
      <div className='flex justify-between flex-wrap items-center items-end mb-2 gap-2'>
        <div className='flex-1 flex justify-center md:justify-end md:ml-auto'>
          <StoneSearch
            ...
            onMinus={stoneId => {
              ...
              setSortedStones(prev =>
                prev.map(s =>
                  s.id === stoneId ? { ...s, samples_amount: newAmount } : s,
                ),
              )
            }}
          />
        ...
        <Button
          variant={colorSort ? 'secondary' : 'outline'}
      ...
      <StoneTable stones={sortedStones} columns={columns} enableHighlight={true} />
