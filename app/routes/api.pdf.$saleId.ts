...
import { concatTransformationMatrix, PDFDocument, PDFForm } from "pdf-lib";
...
const urls = {
  homeownerGDIndy: "https://granite-database.s3.us-east-2.amazonaws.com/base-contracts/Contract.pdf",
  commercialGDIndy: "https://granite-database.s3.us-east-2.amazonaws.com/base-contracts/Contract_commercial.pdf",
};
...
for (let i = 0; i < queryData.length && i < 3; i++) {
  ...
  pdfForm
    ...
    .setText(row.square_feet?.toString() || "N/A");
  pdfForm
    .getTextField(priceField)
    .setText(row.retail_price?.toString() || "N/A");
}
...
const hasLaminateTearOut = queryData.some(
  ...
  pdfForm.getTextField("Text33").setText(stoveText[queryData[0].stove as keyof typeof stoveText] || "N/A");
...
function commercialGdIndyText(pdfForm: PDFForm, queryData: IQuery[]) {
  // Header fields
  pdfForm.getTextField("Text123").setText(queryData[0].sale_date?.toLocaleDateString("en-US") || undefined);
  ...
  // Up to three rooms
  const roomBase = [
    { room: "Text131", color: "Text132", sink: "Text133", edge: "Text134", back: "Text135", sqft: "Text146", price: "Text147" },
    { room: "Text136", color: "Text137", sink: "Text138", edge: "Text139", back: "Text140", sqft: "Text148", price: "Text149" },
    { room: "Text141", color: "Text142", sink: "Text143", edge: "Text144", back: "Text145", sqft: "Text150", price: "Text151" },
  ];
  ...
  for (let i = 0; i < queryData.length && i < 3; i++) {
    ...
    pdfForm.getTextField(map.room).setText(
      row.room ? row.room.charAt(0).toUpperCase() + row.room.slice(1).toLowerCase() : undefined
    );
    ...
    pdfForm
      ...
      .setText(
        row.backsplash
          ? row.backsplash.charAt(0).toUpperCase() + row.backsplash.slice(1).toLowerCase()
          : "N/A"
      );
  ...
  pdfForm.getTextField("Text154").setText(stoveText[queryData[0].stove as keyof typeof stoveText] || "N/A");
  ...
  pdfForm.getDropdown("Dropdown159").select(seamText[seamKey as keyof typeof seamText] || "N/A");

  const fullPrice = queryData[0].total_price || 0;
  const halfPrice = fullPrice * 0.5;
  pdfForm.getTextField("Text160").setText(fullPrice.toString());
  pdfForm.getTextField("Text161").setText(fullPrice > 1000 ? halfPrice.toString() : fullPrice.toString());
...
export async function loader({ request, params }: ActionFunctionArgs) {
  ...
  let contractType = queryData[0].company_name ? "commercialGDIndy" : "homeownerGDIndy"
