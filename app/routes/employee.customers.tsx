...
import { LoaderFunctionArgs, redirect, useSearchParams, useNavigate, useLocation } from "react-router";
...
function SalesRepCell({ customer }: { customer: Customer }) {
  ...
  const form = useForm<{ rep: string }>({
    defaultValues: { rep: customer.sales_rep ? String(customer.sales_rep) : "" },
  });

  const mutation = useMutation({
    mutationFn: async (newRep: string) => {
      const body = { customer_id: customer.id, sales_rep: newRep ? Number(newRep) : null };
