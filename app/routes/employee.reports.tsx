...
export const loader = async ({ request }: LoaderFunctionArgs) => {
  ...
  const stoneId = url.searchParams.get("stoneId") || "";
  ...
  let query = "";
  const queryParams: any[] = [user.company_id]; // Always filter by company_id
    ...
    case "cut_slabs":
      query = `
        ...
        WHERE 
          ...
          AND (${supplierFilter === "" ? "1=1" : "suppliers.id = ?"})
          AND (${stoneFilter === "" ? "1=1" : "stones.id = ?"})
      `;
    ...
    case "sales_by_seller":
      query = `
        SELECT 
          ...
          COUNT(DISTINCT sales.id) as notes,
          'Seller' as status
        FROM 
          sales
        JOIN 
      ...
      query += ` GROUP BY users.id, users.name ORDER BY SUM(slab_inventory.square_feet) DESC`;
      break;
...
export default function ReportsPage() {
  ...
  const getReportTitle = () => {
    switch (reportType) {
      ...
      case "sales_by_seller": return "Sales by Seller Report";
  ...
  const reportTypeOptions = [
    ...
    { key: "sales_by_seller", value: "Sales by Seller" },
