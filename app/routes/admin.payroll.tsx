...
export async function action({ request }: ActionFunctionArgs) {
  ...
  if (intent === "set-payroll") {
    try {
      ...
      const session = await getSession(request.headers.get("Cookie"));
      session.flash("message", toastData("Success", payrollAmount ? "Payroll amount set" : "Payroll marked as unpaid"));
      ...
      const session = await getSession(request.headers.get("Cookie"));
      session.flash("message", toastData("Error", "Failed to update payroll amount", "destructive"));
