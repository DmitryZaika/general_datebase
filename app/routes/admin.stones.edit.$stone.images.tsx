...
function LinkImagesDialog({ allStones, isOpen, onClose, currentStoneId }: LinkImagesDialogProps) {
  ...
  return (
    <Dialog open={isOpen} onOpenChange={(open) => {
      ...
      <DialogContent>
        ...
        <RemixForm method="post" className="flex justify-end gap-2" onSubmit={() => {
          if (selectedStoneId) {
...
export default function SelectImages() {
  const { stones, allStones, linkedImages, currentStoneId } = useLoaderData<typeof loader>();
  ...
  const groupedLinkedImages = linkedImages.reduce<Record<number, { name: string; images: Array<{url: string}> }>>(
    (acc, img) => {
