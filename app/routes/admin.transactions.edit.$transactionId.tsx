    ...
    try {
      if (intent === "sale-unsell") {
        ...
        try {
          ...
          const session = await getSession(request.headers.get("Cookie"));
          session.flash("message", toastData("Success", "Transaction information updated successfully"));
        ...
        const session = await getSession(request.headers.get("Cookie"));
        session.flash("message", toastData("Success", "Customer created and assigned to transaction"));
      ...
      const session = await getSession(request.headers.get("Cookie"));
      session.flash("message", toastData("Error", "Failed to update transaction", "destructive"));
          ...
          <DialogContent className="bg-white rounded-lg p-6 shadow-lg">
            ...
            <div className="my-4">
              <p className="text-sm text-gray-600">
                Are you sure you want to return all items from this transaction back to stock? 
                This will reset all slabs and sinks that were part of this transaction.
              </p>
            </div>
          ...
          <DialogContent className="bg-white rounded-lg pt-4 px-4 shadow-lg text-gray-800 overflow-y-auto max-h-[85vh] max-w-2xl">
            ...
            <Tabs defaultValue="slabs" className="w-full">
              ...
              <TabsContent value="slabs" className="max-h-[40vh] overflow-y-auto rounded-md bg-gray-50 p-3 shadow-inner">
                {slabs.length === 0 ? (
                  <p className="text-center text-gray-500 py-4">No slabs in this transaction</p>
                ) : (
