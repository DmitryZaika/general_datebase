...
export async function action({ request, params }: ActionFunctionArgs) {
  ...
  try {
    if (intent === "sale-unsell") {
      ...
      session.flash(
        "message",
        toastData("Success", "All items processed successfully")
      );
    ...
    if (intent === "slab-delete" || intent === "sink-delete") {
      session.flash(
        "message",
        toastData("Success", "Item removed from transaction")
      );
    ...
    session.flash(
      "message",
      toastData("Success", "Transaction updated successfully")
    );
        ...
        <DialogContent className="bg-white rounded-lg p-6 shadow-lg">
          ...
          <div className="my-4">
            <p className="text-sm text-gray-600">
              Are you sure you want to return all items from this transaction
              back to stock? This will reset all slabs and sinks that were part
              of this transaction.
            </p>
          </div>
