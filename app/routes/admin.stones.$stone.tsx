...
export async function loader({ request, params }: LoaderFunctionArgs) {
  ...
  if (soldSlabIds.length > 0) {
    ...
    const transactionsBySlab = transactionResults.reduce(
      (acc, transaction) => {
    ...
    allSlabs.forEach(slab => {
      ...
      if (slabTransactions && slabTransactions.length > 0) {
        const transaction = slabTransactions[0]
...
export async function action({ request, params }: ActionFunctionArgs) {
  ...
  if (intent === 'updateSize') {
    ...
    await db.execute('UPDATE slab_inventory SET length = ?, width = ? WHERE id = ?', [
      length,
      width,
      slabId,
    ])
        ...
        <Tooltip>
          <TooltipTrigger asChild>
            ...
            >
              <img
                src={
                  slab.url === 'undefined' || slab.url === null ? stone.url : slab.url
                }
                alt='Slab'
                ...
                {slab.transaction ? (
                  <>
                    ...
                    {slab.transaction.sale_notes && (
                      <p>
                        <strong>Notes to Sale:</strong> {slab.transaction.sale_notes}
                      </p>
                    )}

                    {slab.transaction.slab_notes && (
                      <p>
                        <strong>Notes to Slab:</strong> {slab.transaction.slab_notes}
                      </p>
                    )}
                  </>
                ) : slab.parent_transaction ? (
                  ...
                  <>
                    ...
                    <p className='text-xs mt-1'>
                      This slab is marked as sold but has no transaction details.
                    </p>
                  </>
                )}
              </div>
