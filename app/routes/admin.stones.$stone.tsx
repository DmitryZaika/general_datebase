import {    LoaderFunctionArgs,    ActionFunctionArgs,    redirect,    useLoaderData,    Form,    useNavigation,    useNavigate,    data,    Outlet,    Link,    useLocation,    useParams,  } from "react-router";
  ...
  export async function action({ request, params }: ActionFunctionArgs) {
    ...
    export default function SlabsModal() {    const { slabs, linkedSlabs, stone } = useLoaderData<typeof loader>();    const [selectedImage, setSelectedImage] = useState<string | null>(null);    const [editingSlab, setEditingSlab] = useState<number | null>(null);    const [highlightedSlab, setHighlightedSlab] = useState<number | null>(null);    const navigation = useNavigation();    const navigate = useNavigate();    const location = useLocation();    const params = useParams();
            ...
            <TooltipTrigger asChild>
              ...
              >
                ...
                <div className="flex items-center gap-2 w-full">
                  {isEditing ? (
                    <Form method="post" className="flex items-center gap-2 w-full" onClick={(e) => e.stopPropagation()}>
