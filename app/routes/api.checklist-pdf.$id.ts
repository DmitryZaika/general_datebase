...
async function generatePdf(data: ChecklistData): Promise<Uint8Array> {
  ...
  const checkboxLine = (label: string, checked: boolean) => {
    ...
    if (checked) {
      ...
      page.drawLine({
        ...
        end: { x: boxX + 9, y: boxY + 9 },
        thickness: 1,
        color: rgb(0, 0, 0),
      })
      page.drawLine({
        start: { x: boxX + 9, y: boxY + 1 },
        end: { x: boxX + 1, y: boxY + 9 },
        thickness: 1,
        color: rgb(0, 0, 0),
      })
    }
  ...
  const commentMaxWidth = width - 110
  const paragraphs = (data.comments || '').split(/\n|\r/)

  paragraphs.forEach(paragraph => {
    const lines = wrapParagraph(paragraph, fontRegular, 12, commentMaxWidth)
    lines.forEach(line => {
