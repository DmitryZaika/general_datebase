...
export default function CompanyCustomers() {
  ...
  useEffect(() => {
    if (
      searchTerm.length >= 2 &&
      (!selectedCustomer || selectedCustomer.name !== searchTerm)
    ) {
      const timeoutId = setTimeout(() => {
        searchFetcher.load(
          `/api/customers/search?term=${encodeURIComponent(searchTerm)}&searchType=name`,
        )
      }, 300)
      ...
      {customers.length === 0 ? (
        ...
        <div className='grid gap-3'>
          {customers.map(c => {
            ...
            const displayName =
              isCurrentCustomer && c.company_name ? c.company_name : c.name
                  ...
                  <div className='flex justify-between items-start gap-4'>
                    <div className='flex-1 min-w-0'>
                      ...
                      <div className='text-xs text-slate-500 space-y-0.5 mt-1 truncate'>
                        ...
                        {locationValue ? (
                          <div className='truncate'>{locationValue}</div>
                        ) : null}
                      </div>
