...
export const action = async ({ request, params }: ActionFunctionArgs) => {
  ...
  try {
    ...
    if (intent === 'toggle') {
      return handleToggle(formData, dealId, user.company_id, user.id)
    }
...
async function handleToggle(
  ...
  const rows = await selectMany<{ is_completed: number; name: string }>(
    db,
    'SELECT is_completed, name FROM deal_activities WHERE id = ? AND deal_id = ? AND company_id = ? AND deleted_at IS NULL',
    [Number(activityId), dealId, companyId],
  )
