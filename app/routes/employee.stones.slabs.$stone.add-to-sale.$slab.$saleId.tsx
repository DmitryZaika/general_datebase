...
export async function action({ request, params }: ActionFunctionArgs) {
  ...
  try {
    ...
    const [slabCheck] = await db.execute<RowDataPacket[]>(
      `SELECT id, stone_id FROM slab_inventory WHERE id = ?`,
      [slab]
    );
        ...
        <div className="space-y-4 py-4">
          <div className="bg-gray-50 p-3 rounded-md">
            ...
            <div className="text-sm mt-1">
              <p><span className="font-medium">Customer:</span> {saleDetails.customer_name}</p>
              <p><span className="font-medium">Date:</span> {formattedDate}</p>
              <p><span className="font-medium">Current Bundles:</span> {bundlesList}</p>
              {saleDetails.sinks.length > 0 && (
                <p><span className="font-medium">Sinks:</span> {saleDetails.sinks.join(", ")}</p>
              )}
            </div>
          </div>
          ...
          <div className="bg-gray-50 p-3 rounded-md">
            ...
            <div className="text-sm mt-1">
              <p><span className="font-medium">Stone:</span> {slabDetails.stone_name}</p>
              <p><span className="font-medium">Bundle:</span> {slabDetails.bundle}</p>
            </div>
          </div>
        </div>
