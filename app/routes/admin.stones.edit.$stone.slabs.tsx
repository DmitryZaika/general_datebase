...
export async function action({ request, params }: ActionFunctionArgs) {
  ...
  if (request.method === "PATCH") {
    try {
      ...
      session.flash(
        "message",
        toastData("Success", "Slab number updated successfully")
      );
      return data(
        { success: true },
        {
  ...
  // Handle DELETE requests
    ...
    try {
      ...
      if (unlinkSourceId) {
        ...
        session.flash(
          "message",
          toastData("Success", "Slabs unlinked successfully")
        );
        return data(
          { success: true },
          {
    ...
    if (contentType.includes("application/x-www-form-urlencoded")) {
      try {
        ...
        if (intent === "link_slabs") {
          ...
          session.flash(
            "message",
            toastData("Success", "Slabs linked successfully")
          );
          return data(
            { success: true },
            {
      ...
      try {
        ...
        if (formData.length === 0 && formData.width === 0) {
          const [stoneRecord] = await selectMany<{
            width: number;
            length: number;
          }>(db, "SELECT width, length FROM stones WHERE id = ? LIMIT 1", [
            stoneId,
          ]);
...
export async function loader({ request, params }: LoaderFunctionArgs) {
  ...
  const [stone] = await selectMany<{
    ...
    db,
    "SELECT id, width, length, url, name FROM stones WHERE id = ? LIMIT 1",
    [stoneId]
  );
            ...
            <div key={item.source_stone_id} className="mb-6 relative">
              <div className="flex justify-between items-center mb-2">
                ...
                <Button
                  ...
                  onClick={() =>
                    handleUnlinkClick(
                      item.source_stone_id,
                      item.source_stone_name
                    )
                  }
                >
                  <FaTimes size={12} />
