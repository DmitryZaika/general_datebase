...
const { GetObjectCommand, PutObjectCommand } = pkg as {
  GetObjectCommand: any; PutObjectCommand: any;
};
 ...
 */
export async function processAndUploadImage(srcKey: string, destKey: string): Promise<void> {
  // 1) скачать исходник
...
// ──────────────────────────── MAIN ─────────────────────────────────────────
async function main() {
  const [rows] = await db.query<{ url: string }[] & RowDataPacket[]>('SELECT url FROM stones')
  ...
  for (const file of missingFiles) {
    const cleanFile = file.replace('https://granite-database.s3.us-east-2.amazonaws.com/', '');
